...
{- | Given a podcast an d an PodItem, produce an Episode -}
...
{- | Convert [Content] to a printable string, takeing care to unescape it.
An implementation without unescaping would simply be:
> contentToString = concatMap (show . content)
Because HaXml's unescaping works only on Elements, we must take sure that
whatever Content we have is wrapped in an Element, then sue txt to
