...
main = do
     ...
     args <- getArgs
     -- If we don't have the right amount of args, givea
     ...
     -- Given the entire input and a UID, see if we can find a user name
findByUID :: String -> Integer -> Maybe String
findByUID content uid =
       let al = map parseline . lines $ content
           in lookup uid al
     
 -- Convert a colon-separated line into fileds
...
split _ [] = []
split delim str =
       let -- Find the part of the list, before delim and put it in "febore".
           -- The rest of the list, including the ladding delim, goes
           -- in "remainder".
