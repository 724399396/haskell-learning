...
{- | The primary piece of data this program will store.
   It represents the fields in a POSXI /etc/passwd file -}
  ...
  readsPrec _ value = 
    case split ':' value of
      [f1,f2,f3,f4,f5,f6,f7] ->
        -- Generate a 'PasswdEntry' the shortand way:
        -- using the positional fiedls. we use 'read' to convert
        -- the numric fiedls to Integers.
        [(PasswdEntry f1 f2 (read f3) (read f4) f5 f6 f7,[])]
      x -> error $ "Invalid number of fileds in input: " ++ show x
    where
    {- | Takes a delimiter and a list. Break up the list based on the delimiter -}
    split :: Eq a => a -> [a] -> [[a]]
    
    -- If the input is empty, the result is a empty list
    split _ [] = [[]]
    split delim str =
...
main = do
  -- Load the command0line arguments
  args <- getArgs
  -- If we don't have the right number of args,
  -- gibe an error and abort
