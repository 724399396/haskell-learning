...
main = do
  ...
  -- one therad reports bad links to stdout
...
worker badLinks jobQueue badCount = loop
  where
    ...
    checkOne url = case parseURI url of
      Just uri -> do
        code <- getStatus uri `catchAny` (return . Left . show)
        case code of
          Right 200 -> return ()
...
options = [ Option ['h'] ["help"] (NoArg Help)
            ...
            Option ['n'] []       (ReqArg (\s -> N (read s)) "N")
                   "Number of concurrent connections (defualt 16)" ]
